...
describe "Create payment on successful charge"  do
  ...
  it "[creates a payment associated with the user", :vcr do
    bob = Fabricate(:user, customer_token: "cus_5222Vc3jpmZtoi")
    post "/stripe_events", event_data
    expect(Payment.first.user).to eq(bob)
  end

  it "[create the payment with the ammount]", :vcr do
    bob = Fabricate(:user, customer_token: "cus_5222Vc3jpmZtoi")
    post "/stripe_events", event_data
    expect(Payment.first.ammount_in_cents).to eq(999)
  end

  it "[create the payment with reference id]", :vcr do
    bob = Fabricate(:user, customer_token: "cus_5222Vc3jpmZtoi")
    post "/stripe_events", event_data
    expect(Payment.first.reference_id).to eq("ch_14rxyWAQtNZoyYMJYtqLJWpo")
  end
end